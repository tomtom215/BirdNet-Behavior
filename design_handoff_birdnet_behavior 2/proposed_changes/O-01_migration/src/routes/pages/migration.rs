...
async fn ridgeline_partial(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let result = tokio::task::spawn_blocking(move || {
        state.with_db(|conn| collect_ridges(conn, year, 12))
    })
    .await;
...
async fn card_partial(
    ...
    let (eyebrow, headline, sub) = match q_kind(&result, &q.kind) {
        ...
        None => match q.kind.as_str() {
            ...
            "peaking" => ("Currently peaking", "Quiet week".into(), "No species over 10 detections".into()),
...
fn q_kind(
    ...
    match kind {
        "arrived" => Some(("Just arrived", row.0.clone(), format!("First heard {}", row.1))),
        "peaking" => Some((
            "Currently peaking",
            row.0.clone(),
