...
say "  ${BLD}Doctor:${RST}   cd $(pwd) && docker compose exec birdnet birdnet-behavior --doctor"
...
say ""
say "If something looks wrong, run the diagnostic any time:"
say "  ${BLD}docker compose exec birdnet birdnet-behavior --doctor${RST}"
say "It prints a one-screen report (audio source, model, disk, network, config)"
say "with a suggested fix for every problem it finds."
say ""
