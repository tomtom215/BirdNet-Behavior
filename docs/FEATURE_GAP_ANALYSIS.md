...
| **And a gate the change had to add** | That made `/admin/audio` a third writer of the `settings` table, and the guard that fails when a settings key nothing reads is shipped covered only the admin form and the first-run wizard — the two writers whose past mistakes created it. `AUDIO_ADMIN_SETTING_KEYS` and `audio_admin_keys_are_all_classified` extend it to this one. `routes/admin/migration.rs` writes `exclude_imports` and is still outside that guard; closing that properly is G‑34's drift gate. |
...
| **Plan** | The breaker is done. Enqueue to `outbound_queue` while a circuit is open instead of dropping, so the breaker and the store-and-forward queue compose rather than compete. Add `*_file` indirection for every credential the dispatcher reads, since that is a small change with a real deployment payoff. The script provider is declined: arbitrary command execution configured through the web UI is the same RCE surface as the declined web terminal. |
...
| 12 | Notification `*_file` secrets + enqueue-while-open (the circuit breaker itself shipped, `80bc37e`) | G‑28 |
