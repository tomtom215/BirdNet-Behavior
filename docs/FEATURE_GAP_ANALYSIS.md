...
#### G‑30 · Backup destinations — PARTIAL (the `rsync` target has shipped)
...
| **What remains** | The daily schedule. It is not simply a shorter interval: the weekly job is backup → offsite → prune → **space reclaim**, and running the reclaim daily would rewrite the database file every day on an SD card. Doing this properly means separating the reclaim into its own weekly job, which is the next commit rather than a footnote in this one. |
