//! Command palette server-side filter. See O-19 DIFF.md.
//!
...
async fn cmdk(State(state): State<AppState>, Query(qp): Query<CmdkQuery>) -> Html<String> {
    ...
    if q.is_empty() {
        // Default view: a curated "Jump to…" list + recent detections.
...
fn render_group(out: &mut String, title: &str, rows: &[Entry]) {
    ...
    let _ = write!(out, r#"<li class="bnb-cmdk__group">{}</li>"#, escape_html(title));
    for r in rows {
        let _ = write!(
            out,
...
fn all_pages() -> Vec<Entry> {
    ...
    vec![
        ...
        make("◐", "Dawn chorus",      "polar plot",      "/analytics/dawn-chorus", &["chorus", "polar", "circadian"]),
        make("∿", "Migration",        "phenology",       "/migration",       &["arrivals", "ridgeline", "departures"]),
        make("☰", "Correlation",      "co-occurrence",   "/correlation",     &["matrix", "pairs"]),
        ...
        make("▶", "Recordings",       "clips",           "/recordings",      &["audio", "wav", "clip"]),
        make("⚠", "Quarantine",       "review queue",    "/quarantine",      &["rare", "queue", "review"]),
        make("¶", "Weekly report",    "Sunday recap",    "/weekly",          &["bulletin", "sunday"]),
        make("⊞", "Year in review",   "annual recap",    "/year-in-review",  &["year", "annual"]),
        make("◉", "Kiosk",            "wall display",    "/kiosk",           &["ambient", "wall"]),
        make("≡", "Notifications",    "channels & log",  "/notifications",   &["alerts", "channel"]),
...
// ---------------------------------------------------------------------------
// Index — Settings (admin sub-pages with synonyms)
// ---------------------------------------------------------------------------
...
fn all_settings() -> Vec<Entry> {
    ...
    vec![
        make("Detection",       "settings", "/admin/settings",     &["threshold", "sensitivity", "confidence"]),
        make("Audio",           "sources",  "/admin/audio",        &["microphone", "mic", "rtsp", "usb", "alsa", "pipewire"]),
        make("Notifications",   "channels", "/admin/notifications", &["telegram", "email", "mqtt", "slack", "webhook"]),
        make("Species",         "filter",   "/admin/species",      &["exclude", "include", "allow", "list"]),
        make("Rules",           "alerts",   "/admin/rules",        &["alert", "rule"]),
        make("Quality",         "metrics",  "/admin/quality",      &["false positive", "low confidence"]),
        make("Backups",         "recovery", "/admin/backups",      &["backup", "restore", "snapshot"]),
        make("Migrate",         "import",   "/admin/migration",    &["birdnet-pi", "import"]),
        make("Diagnostics",     "doctor",   "/admin/doctor",       &["health", "self-check"]),
        make("Display",         "prefs",    "/system#display-prefs", &["theme", "density", "motion", "contrast"]),
    ]
}
    ...
    tokio::task::spawn_blocking(move || {
        state2.with_db(|conn| {
            ...
            let mut stmt = match conn.prepare(
                "SELECT common_name, scientific_name, count
                   FROM species_aggregate
            ...
            let rows = stmt.query_map((&qlc, limit as i64), |row| {
                Ok((
                    row.get::<_, String>(0)?,
                    row.get::<_, String>(1)?,
                    row.get::<_, i64>(2)?,
            ...
            if let Ok(it) = rows {
                for r in it.flatten() {
                    ...
                    let _ = sci; // future: index banding code
                }
            }
            out
        })
...
async fn render_recent(out: &mut String, state: &AppState) {
    let state2 = state.clone();
    let recent: Vec<(String, String, String)> = tokio::task::spawn_blocking(move || {
        state2
            .with_db(|conn| {
                let mut rows = Vec::new();
                let Ok(mut stmt) = conn.prepare(
                    "SELECT common_name, scientific_name, Date || ' ' || Time AS at
                       FROM detections
                ...
                if let Ok(it) = it { for r in it.flatten() { rows.push(r) } }
                rows
            })
    })
    .await
    ...
    let entries: Vec<Entry> = recent.into_iter().map(|(com, _sci, at)| Entry {
        glyph: "♪",
        label: com.clone(),
        sub: at,
...
// ---------------------------------------------------------------------------
// Date parser — small, pure, deterministic
// ---------------------------------------------------------------------------
...
fn parse_date(q: &str) -> Option<Entry> {
    ...
    // YYYY-MM-DD
    if q.len() == 10 && super::is_valid_date(&q) {
        return Some(Entry {
            glyph: "◷",
            label: q.clone(),
            sub: "history".into(),
    ...
    let (label, sub, href) = match q.as_str() {
        ...
        "yesterday"  => ("Yesterday",      "history",       format!("/history?date={}", shift_date(&today, -1))),
        "this week"  => ("This week",      "weekly report", "/weekly".to_string()),
        "last week"  => ("Last week",      "weekly report", "/weekly?offset=-1".to_string()),
        "this month" => ("This month",     "trends",        "/timeseries?period=month".to_string()),
        "last month" => ("Last month",     "trends",        "/timeseries?period=month&offset=-1".to_string()),
...
/// Shift a YYYY-MM-DD by `days` (negative = earlier). Cheap, no chrono dep.
fn shift_date(date: &str, days: i32) -> String {
    ...
    // Compute days-since-epoch using the Hinnant algorithm in reverse.
    // Simpler: walk days forward/back day-at-a-time — bounded by |days| ≤ ~31.
...
/// Inverse of `days_to_date` (Hinnant).
...
mod tests {
    ...
    fn pages_index_has_all_topnav_links() {
        ...
        for must in ["Dashboard","Today","Species","Heatmap","Migration","Life list","Quarantine","System"] {
            assert!(labels.iter().any(|l| *l == must), "missing {must} in cmdk pages index");
        }
    }
    ...
    fn shift_date_roundtrip() {
        // shift forward then back returns the same string.
        let d = "2025-05-15";
