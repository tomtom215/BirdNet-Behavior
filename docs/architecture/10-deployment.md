...
# Or with the distro cross toolchain, which is what CI uses
sudo apt install gcc-aarch64-linux-gnu g++-aarch64-linux-gnu binutils-aarch64-linux-gnu
cargo build --release --target aarch64-unknown-linux-gnu
```

`cargo-zigbuild` is **not** an option here: Zig's `-lstdc++` does not
provide the full GNU cxx11-ABI symbol set that pyke's prebuilt ONNX
Runtime archives reference, so the link fails. See `RELEASING.md`.
...
The CI workflow (`.github/workflows/ci.yml`) runs eight jobs on every push
and pull request:
...
4. **inference** — the end-to-end suites against the real model
5. **doc** — `cargo doc --workspace --no-deps --document-private-items
   --all-features` with broken-link denial
6. **build** — debug build with and without the `analytics` feature
...
The unit is **generated by the installer** — `installer/lib/65-service.sh`
is the source of truth, and there is no `.service` file in the tree to
copy. The load-bearing directives it writes:
...
# No permanent give-up: the rate limit is off entirely, and there is no
# StartLimitBurst. A tight restart loop is handled by the backoff below.
...
# Preflight: exit 2 (errors) blocks startup; 0 and 1 both proceed.
...
`After=` is ordering only. Nothing `Wants=` or `Requires=`
`time-sync.target`, so the unit starts whether or not the clock ever
syncs; the daemon fails open on clock-dependent gating instead.

Hardening is **not optional** — the installer always writes it, and the
set is larger than the block above: `ProtectSystem=strict`,
`ProtectHome=read-only`, an explicit `ReadWritePaths=`, an empty
`CapabilityBoundingSet=`, `UMask=0027`, `RestrictAddressFamilies=`, two
`SystemCallFilter=` lines, and `DeviceAllow=` for ALSA under
`DevicePolicy=closed`. Read `installer/lib/65-service.sh` rather than
transcribing it.

### Install the service

```bash
sudo bash install.sh          # writes the unit, reloads, enables and starts it
...
1. CLI flags (`--listen`, `--config`, `--analytics-db`)
2. Environment variables — every flag has a `BIRDNET_`-prefixed
   equivalent (`BIRDNET_LISTEN`, `BIRDNET_CONFIG`, …); run `--help` for
   the current list
