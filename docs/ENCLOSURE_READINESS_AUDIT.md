...
guessing. Two conclusions I reached early and then disproved are marked
...
`ffmpeg`, `sox` and `procps` (which provides `/bin/kill` on Debian) are not
installed either. So in the shipped image:
...
* The admin Restart button calls `Command::new("kill")` and discards the result
  (`let _ = … .status()`), so it silently does nothing.
...
* Four hand-rolled leap-year predicates survive (`birdnet-scheduler/src/solar.rs:229`,
  `birdnet-web/src/routes/pages/history.rs:545`,
  `birdnet-web/examples/screenshot_server.rs:69`, `src/capture/schedule.rs:473`)
  and two `days_in_month` (`solar.rs:235`, `history.rs:217`), alongside the
  consolidated `birdnet_core::civil`. This is `FIELD_READINESS_AUDIT.md` F-13's
  remaining tail, still open; naming the exact six is the useful part.
