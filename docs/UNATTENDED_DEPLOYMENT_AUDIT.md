...
| **S-4** | P1 | READ | Clip WAVs are written straight to their final path — a torn write leaves a truncated file the database points at for ever. Same as **PS-7**. | See **PS-7**. **[FIXED]** (`S4HASH`, 2026-09-08): `birdnet_core::atomic_file` — write to `name.part.ext`, `fsync`, `rename`, `fsync` the directory — and `write_wav_clip` and the ffmpeg/sox conversion commit through it. Gate `wav::tests::a_kill_mid_write_leaves_nothing_under_the_final_name` spawns the test binary as a worker writing a 600 s clip, kills it the moment a file appears, and asserts the final name is absent or a whole WAV; with the write made direct it failed with a 0-byte file under the final name. |
