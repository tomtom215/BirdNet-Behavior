...
| **R-2** | P1 | READ | **The threshold in force is never stored** — `src/daemon/processor.rs:544` writes `cutoff: None` — while `dynamic_thresholds` (migration 38) moves it per species mid-season. A species' apparent rise is indistinguishable from its threshold falling. | Same table. |
...
| **UP-1** | P1 | **Both references stamp provenance on every detection row and we write NULL.** `src/daemon/processor.rs:542-547` inserts `lat: None` (`:542`), `lon: None` (`:543`), `cutoff: None` (`:544`), `sensitivity: None` (`:546`), `overlap: None` (`:547`) — only `week` (`:545`) is filled — and the same record is mirrored into DuckDB (`:683`), so both stores carry the NULLs. BirdNET-Pi writes all five (`scripts/utils/reporting.py:97-100`); birdnet-go persists four (`internal/detection/factory.go:51-54`). We inherited the schema *and* export its header (`export/csv.rs:173`), so every "BirdNET-Pi-compatible" CSV we emit has five permanently empty columns. Worse for us than for them, because our dynamic thresholds move the rule mid-season. (`dad46d1` gave the eBird export coordinates from settings at export time; the rows are unchanged.) |
