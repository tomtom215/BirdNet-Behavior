...
- **Hand-rolled error types** in libraries (no `anyhow`/`thiserror`), with `unsafe` forbidden workspace-wide (`unsafe_code = "forbid"`), `missing_docs` enforced, and Clippy pedantic + nursery denied in CI.
