...
> The richest DuckDB-powered views — activity sessions, species retention, next-species prediction, year-on-year trends — light up when you run the **analytics** build (`:latest-analytics` image, or `--features analytics`). Without it, these panels explain what they'd show and how to enable them. See [Configuration](../getting-started/configuration.md).
