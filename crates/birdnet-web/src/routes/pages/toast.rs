...
/// Toast tone.
#[derive(Clone, Copy)]
// Complete tone surface; the `Info` variant is part of the API but not yet
// constructed by any wired endpoint.
#[allow(dead_code)]
pub enum Kind {
    Success,
    Warn,
...
impl Toast {
    ...
    // `Info` constructor is part of the complete tone API; no wired endpoint
    // emits info toasts today.
    #[must_use]
    #[allow(dead_code)]
    ...
    // Optional API; current call sites rely on the per-kind defaults.
    #[must_use]
    #[allow(dead_code)]
...
// Plain `&str` helpers for handlers composing HTML by hand. Current call sites
// use the structured `with` / `oob_only` API; these are kept for parity with
// the documented helper surface.
#[must_use]
#[allow(dead_code)]
