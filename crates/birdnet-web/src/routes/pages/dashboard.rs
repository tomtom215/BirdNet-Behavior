...
async fn top_species_partial(State(state): State<AppState>) -> impl axum::response::IntoResponse {
    ...
    match result {
        Ok(Ok(species)) => {
            ...
            if species.is_empty() {
                html = r#"<p style="color:var(--text-muted)">No species detected yet.</p>"#.to_string();
            }
            (StatusCode::OK, [(header::CONTENT_TYPE, "text/html")], html)
        }
        _ => (
...
async fn species_list_partial(
    ...
    match result {
        Ok(Ok(species)) => {
            if species.is_empty() {
                let msg = if has_search { "No matching species found." } else { "No species detected yet." };
            ...
            for s in &species {
                ...
                let _ = write!(html,
                    ...
                    n = escape_html(&s.com_name), c = s.count,
                );
            }
            html.push_str("</tbody></table>");
            (StatusCode::OK, [(header::CONTENT_TYPE, "text/html")], html)
        }
        _ => (StatusCode::INTERNAL_SERVER_ERROR, [(header::CONTENT_TYPE, "text/html")],
...
async fn hourly_chart_partial(State(state): State<AppState>) -> impl axum::response::IntoResponse {
    ...
    .await;
    match result {
        Ok(Ok(hours)) => (StatusCode::OK, [(header::CONTENT_TYPE, "text/html")], render_hourly_chart(&hours)),
        _ => (StatusCode::INTERNAL_SERVER_ERROR, [(header::CONTENT_TYPE, "text/html")], "<p>Error loading chart</p>".to_string()),
    }
}
...
async fn daily_chart_partial(State(state): State<AppState>) -> impl axum::response::IntoResponse {
    ...
    .await;
    match result {
        Ok(Ok(days)) => (StatusCode::OK, [(header::CONTENT_TYPE, "text/html")], render_daily_chart(&days)),
        _ => (StatusCode::INTERNAL_SERVER_ERROR, [(header::CONTENT_TYPE, "text/html")], "<p>Error loading chart</p>".to_string()),
    }
}

async fn confidence_chart_partial(State(state): State<AppState>) -> impl axum::response::IntoResponse {
    let result = tokio::task::spawn_blocking(move || {
        state.with_db(birdnet_db::sqlite::confidence_distribution)
    })
    .await;
    match result {
        Ok(Ok(buckets)) => (StatusCode::OK, [(header::CONTENT_TYPE, "text/html")], render_confidence_chart(&buckets)),
        _ => (StatusCode::INTERNAL_SERVER_ERROR, [(header::CONTENT_TYPE, "text/html")], "<p>Error loading chart</p>".to_string()),
    }
}

fn conf_class(pct: f64) -> &'static str {
    if pct >= 80.0 { "high" } else if pct >= 50.0 { "mid" } else { "low" }
}
